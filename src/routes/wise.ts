...
async function handle_request(
  ...
  try {
    ...
    const path = params["*"]?.replace(/{{profileId}}/g, nvs.wise.profile_id);
    const body = await request.text();
    const payload = body?.replace(/"{{profileId}}"/g, nvs.wise.profile_id);

    const h = copy_headers(request.headers, [
      "accept-minor-version",
      "content-type",
    ]);
    h.set("authorization", `Bearer ${nvs.wise.api_token}`);

    const res = await fetch(`${nvs.wise.api_url}/${path}${url.search || ""}`, {
      method: METHOD,
