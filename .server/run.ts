...
import { whh_alchemy } from "./api-webhooks/alchemy";
import { whh_anvil } from "./api-webhooks/anvil";
import { webhooks_api } from "./api-webhooks/api";
import { whh_chariot } from "./api-webhooks/chariot";
import { whh_nowpayments } from "./api-webhooks/nowpayments";
import { whh_paypal } from "./api-webhooks/paypal";
import { whh_stripe } from "./api-webhooks/stripe";
...
export default function (s: TStage): Record<string, any> {
  ...
  const q = {
    donation_settlement: qs.donation_settlement(s, links),
    donation_success: qs.donation_success(s, links),
  };
  links.push(...Object.values(q));

  const wh = {
    stripe: whh_stripe(links),
    paypal: whh_paypal(links),
    alchemy: whh_alchemy(links),
    anvil: whh_anvil(links),
    chariot: whh_chariot(links),
    nowpayments: whh_nowpayments(links),
  };
  const wh_api = webhooks_api(s, wh);
  ...
  const q_outputs = Object.fromEntries(
    Object.entries(q).map(([key, queue]) => [`queue_${key}`, queue.url])
  );
